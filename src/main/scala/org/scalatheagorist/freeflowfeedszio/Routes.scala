...
object Routes {
  val layer: ZLayer[Configuration & RSSService, Nothing, Routes] =
    ZLayer {
      (ZIO.service[RSSService], ZIO.serviceWith[Configuration](_.pageSize)).mapN {
        (rssService, pageSize) =>
          new Routes {
            override def apply: Http[Any, Throwable, Request, Response] = {
              ...
              Http.collectZIO[Request] {
                case Method.GET -> Root / "efmagazin" / page =>
                  ZIO.logInfo(s"GET: /efmagazin/$page") *> pullStream(Some(page), Some(pageSize), Publisher.EFMAGAZIN.some, None, None)
                case Method.GET -> Root / "freiheitsfunken" / page =>
                  ZIO.logInfo(s"GET: /freiheitsfunken/$page") *> pullStream(Some(page), Some(pageSize), Publisher.FREIHEITSFUNKEN.some, None, None)
                case Method.GET -> Root / "misesde" / page =>
                  ZIO.logInfo(s"GET: /misesde/$page") *> pullStream(Some(page), Some(pageSize), Publisher.MISESDE.some, None, None)
                case Method.GET -> Root / "schweizermonat" / page =>
                  ZIO.logInfo(s"GET: /schweizermonat/$page") *> pullStream(Some(page), Some(pageSize), Publisher.SCHWEIZER_MONAT.some, None, None)
                case Method.GET -> Root / "german" / page =>
                  ZIO.logInfo(s"GET: /german/$page") *> pullStream(Some(page), Some(pageSize), None, Lang.DE.some, None)
                case Method.GET -> Root / "english" / page =>
                  ZIO.logInfo(s"GET: /english/$page") *> pullStream(Some(page), Some(pageSize), None, Lang.EN.some, None)
                case req@Method.GET -> Root / "search" =>
                  val query = req.url.queryParams.get("term").map(_.mkString)
                  ZIO.logInfo(s"GET: /search/${query.mkString}") *> pullStream(None, None, None, None, query)
                case Method.GET -> Root / "articles" / page =>
                  ZIO.logInfo(s"GET: /articles/$page") *> pullStream(Some(page), Some(pageSize), None, None, None)
                case _ =>
