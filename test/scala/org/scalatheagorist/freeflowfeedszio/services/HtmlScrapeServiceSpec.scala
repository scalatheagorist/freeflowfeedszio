  ...
  override def spec = suite("HtmlScrapeService")(
    suite("stream") {
      test("must work") {
        ...
        httpClient.get(*, *) returns ZIO.succeed(Response(status = *, headers = *, body = Body.fromString(htmlResponse))).provideLayer(clientLayer)
        fileStoreClient.saveIntoDir[Client, RSSFeed](*) returns ZStream.unit.provideLayer(clientLayer)
